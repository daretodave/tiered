---
show: summer-house
number: 6
title: "The Reset Summer"
take_h2: "A new house, new energy."
shape_h2: "Seventeen episodes, three new faces."
canonical_position: 6
premiere_date: "2022-01-17"
location: "The Hamptons, New York"
ep_count: 17
eyebrow: "Premiered January 2022 · Bravo"
lede: "Summer House's sixth season moves into a new Hamptons house — the first relocation since the group settled into Watermill years before — with three new faces joining the main cast and the last of the season-two era's arrivals departing for good."
pull: "A new house, a mostly new energy, after the pandemic forced the format to run without one for a year."
format_summary: "Docusoap · new Hamptons house"
format_caption: "relocation after several years in Watermill"
filming_caption: "The Hamptons · a new house after the Watermill years"
premiere_caption: "Bravo · January 2022"
episodes_caption: "Seventeen episodes, closing on the format's first two-part finale"
watch_list:
  - episode_label: "Format · a new house"
    body: "The group relocates to a new Hamptons house, the first move since the show settled into Watermill years earlier. Watch for how a new setting resets the group's established rhythms."
  - episode_label: "Cast · three new faces"
    body: "Three new members join the main cast this season, the biggest single-season addition since the show's sophomore year. Watch for how they find their footing in an already-established group."
  - episode_label: "Finale · a two-part close"
    body: "The season closes with a two-part finale, a longer send-off than the show has used before. Watch for how the extended format changes the season's final rhythm."
---

Summer House's sixth season moves into a new Hamptons house, its first relocation since the show settled into Watermill years earlier. Three new faces join the main cast, and the last of the season-two arrivals departs the show for good. Seventeen episodes and a two-part finale give the reset group a full summer to settle in, after a prior season that broke from the format entirely.

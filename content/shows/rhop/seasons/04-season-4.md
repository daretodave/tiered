---
show: rhop
number: 4
slug: season-4
title: "Season 4"
canonical_position: 2
premiere_date: "2019-05-05"
location: "Potomac, Maryland"
ep_count: 21
eyebrow: "Aired May–September 2019 · Bravo"
lede: "Season four runs with the exact same six housewives as Season 3 — no new addition, no departure — the first time RHOP holds its full-time cast steady from one season to the next. Katie Rost returns briefly in a guest capacity, but the core group carries the season on its own."
pull: "The first fully unchanged cast in RHOP history lets Season 4 run on chemistry the show has already spent two seasons building."
take_h2: "A test of chemistry alone."
shape_h2: "Twenty-one episodes, no cast turnover."
premiere_caption: "Bravo · May 2019"
filming_caption: "Potomac · the first fully returning full-time cast"
format_summary: "Social reality"
format_caption: "Twenty-one episodes, zero cast turnover"
cast_size: 6
cast_size_caption: "Bryant, Darby, Dixon, Huger, Samuels, Dillard — unchanged from Season 3"
---

Season four is RHOP's first season with zero full-time cast turnover — the same six housewives from Season 3 return with nobody new added and nobody gone. Katie Rost reappears briefly as a guest, but the core group carries the twenty-one episodes on established chemistry alone. It's the clearest test yet of whether the format holds up without a new face to reset the group's dynamics.

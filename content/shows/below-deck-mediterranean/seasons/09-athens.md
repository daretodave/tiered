---
show: below-deck-mediterranean
number: 9
title: "Athens"
canonical_position: 6
premiere_date: "2024-06-03"
location: "Athens, Greece"
host: "Sandy Yawn"
ep_count: 17
eyebrow: "Athens — franchise-first Greek capital waters on the M/Y Purpose"
lede: "Season 9 brought Below Deck Mediterranean to Athens — the Greek capital's coastline a genuine franchise first. The M/Y Purpose suited the Aegean scale, and a 17-episode run found enough cast texture to make the new location register."
pull: "A genuine franchise first in Athens — the location earns its place; the cast chemistry is still finding its ceiling."
format_summary: "Charter yacht · Aegean Sea"
format_caption: "Superyacht charter, Athens and Greek coastal waters"
host_caption: "{seasonOrdinalWord} season with Captain Sandy Yawn at the helm"
filming_caption: "Athens, Greece"
premiere_caption: "Bravo · June 2024"
---

Season 9 marked the franchise's first time filming in Athens — the Greek capital's coastline giving the production geography it hadn't used before. The M/Y Purpose suited the Aegean scale. Captain Sandy's command held the format's structure. The cast brought enough interpersonal texture to sustain 17 episodes. A notable franchise expansion that lands in the honest middle of the canon — the location is the argument; the cast is still developing one.

---
show: below-deck
number: 4
title: "U.S. Virgin Islands"
canonical_position: 10
premiere_date: "2016-09-01"
location: "U.S. Virgin Islands, Caribbean"
host: "Lee Rosbach"
ep_count: 14
eyebrow: "Solid ground, lower ceiling"
take_h2: "Solid, but never quite igniting."
shape_h2: "Fourth straight season, fourteen episodes."
lede: "Season four moved to the U.S. Virgin Islands and delivered a reliable charter run. Captain Lee held the command deck with his characteristic directness, the interior and deck departments generated their usual friction, and the St. John waters made for beautiful footage."
pull: "Functional Below Deck — the format working reliably even when the cast chemistry doesn't fully ignite."
format_summary: "Charter yacht · USVI"
format_caption: "Superyacht charter, U.S. Virgin Islands waters"
host_caption: "{seasonOrdinalWord} season with Captain Lee Rosbach at the helm"
filming_caption: "St. John, U.S. Virgin Islands"
premiere_caption: "Bravo · 2016"
---

Season four moved to the U.S. Virgin Islands and delivered a solid charter run without quite reaching the heights of the Bahamas season. Captain Lee held the command deck with the same authority, the interior-versus-deck tension produced familiar friction, and the St. John waters offered a beautiful staging ground. Competent, watchable, occasionally tense — a season that shows the format working reliably even when the cast chemistry doesn't spark all the way through.

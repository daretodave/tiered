---
show: below-deck
number: 7
title: "Thailand"
canonical_position: 4
location: "Phuket, Thailand"
host: "Captain Lee Rosbach"
premiere_date: "2019-10-01"
ep_count: 20
aired_year: 2019
eyebrow: "Season 7 · Phuket, Thailand"
lede: "Below Deck left the Atlantic basin entirely and anchored in Phuket — a first for the franchise, and a setting that brought a genuinely different texture to the charter format."
pull: "Thailand pushed the franchise beyond its Caribbean frame and proved the format traveled well."
format_summary: "Superyacht charter · Southeast Asia"
format_caption: "Superyacht charter, Phuket waters"
host_caption: "Captain Lee Rosbach's {seasonOrdinal} season at the helm"
filming_caption: "Phuket, Thailand · franchise's first Southeast Asia shoot"
premiere_caption: "Bravo · 2019"
---

Thailand brought Below Deck to Southeast Asia for the first time, anchoring in the Andaman Sea off Phuket with its distinctive limestone islands and aquamarine bays. The location gave the season a visual texture no Caribbean run could replicate, and the twenty-episode order allowed the crew dynamics to develop across a longer arc than most seasons. Charter guests navigated an unfamiliar region, sharpening the service pressure in distinct ways. Captain Lee held command across the franchise's longest season.

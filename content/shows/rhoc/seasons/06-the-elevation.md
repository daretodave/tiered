---
show: rhoc
number: 6
slug: the-elevation
title: "The Elevation"
canonical_position: 3
premiere_date: "2011-04-11"
location: "Orange County, California"
ep_count: 21
eyebrow: "Aired spring 2011 · Peak era begins, new cast shifts the show's social register"
lede: "Season six is where RHOC changes registers. Heather Dubrow and Peggy Tanous join a cast running in a more aspirational Orange County, and the show's social architecture shifts upward to match. The peak era opens with a cast that has genuine chemistry and competing ambitions."
pull: "The OC social world steps up in class and ambition — and the show is sharp enough to use both."
premiere_caption: "Bravo · April 2011"
filming_caption: "Orange County, California"
format_summary: "Social reality"
format_caption: "Peak-era cast configuration, two new additions"
cast_size: 8
cast_size_caption: "Eight cast members, peak-era lineup begins"
---

Season six marks the start of RHOC's peak era with a cast reconfiguration that shifts the show's social world upward. Heather Dubrow's arrival changes the economic register of the group and gives the dynamics a new axis of tension — aspiration, taste, and competing versions of what it means to do well in Orange County. The production is fluent, the cast chemistry is immediate, and the format uses the new social geometry with purpose.

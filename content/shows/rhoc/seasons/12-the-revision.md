---
show: rhoc
number: 12
slug: the-revision
title: "The Revision"
canonical_position: 15
premiere_date: "2017-07-10"
location: "Orange County, California"
ep_count: 20
eyebrow: "Aired summer 2017 · Transition era begins, cast searches for its next shape"
lede: "Without Heather Dubrow, the peak-era social register drops. Lydia McLaughlin returns, Peggy Sulahian joins, and the show begins searching for its next configuration. Genuine chemistry appears in places; the working unit isn't assembled yet."
pull: "The peak era gives way — the show searching for the cast it's becoming."
take_h2: "The cast configuration still unsettled."
shape_h2: "Twenty episodes, one fewer again."
premiere_caption: "Bravo · July 2017"
filming_caption: "Orange County · transition era begins, cast in flux"
format_summary: "Social reality"
format_caption: "Transition-era cast, one return, one new addition"
cast_size: 6
cast_size_caption: "Six cast members, one returning, one joining, one departing"
---

Season twelve is the transition era's opening move. Without Heather Dubrow, the peak-era social register drops and the show begins searching for its next configuration. Lydia McLaughlin returns but reads differently outside the ensemble that originally framed her; Peggy Sulahian's addition generates moments without producing the chemistry the cast needs. The season is genuine where it works and unsettled where it doesn't — a show mid-construction.

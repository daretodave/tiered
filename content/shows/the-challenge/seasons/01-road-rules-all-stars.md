---
show: the-challenge
number: 1
canonical_position: 10
title: Road Rules All Stars
eyebrow: "Aired summer 1998 · The franchise's first crossover"
lede: "The season that started it all. MTV pulled veterans from the first three Road Rules casts, put them in an RV, and turned a road trip into a mission-driven competition. The template for everything the franchise would later become."
format_summary: "Road Rules vets · 1 RV"
format_caption: "all-returnee crossover, pre-format"
cast_size: 7
cast_size_caption: "veterans from RR seasons 1–3"
premiere_caption: "MTV · summer 1998"
filming_caption: "an RV crossing the American West"
shape_h2: "Seven cast, first crossover."
take_h2: "The franchise's rough draft."
---

MTV's first attempt to spin Road Rules into something more
competitive. Seven veterans from the first three Road Rules
casts climbed back into an RV and chased missions across the
American West, with a charity prize on the line. The pacing is
loose, the format barely exists yet, and the show is still
finding what makes returnees compelling on camera. Watch it as
the rough draft of every season that follows.

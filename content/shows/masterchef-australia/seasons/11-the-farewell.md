---
show: masterchef-australia
number: 11
title: "The Farewell"
premiere_date: 2019-07-28
ep_count: 61
location: Melbourne, Brisbane, Noosa, Rottnest Island, Australia
host: Gary Mehigan, George Calombaris, Matt Preston
canonical_position: 12
eyebrow: "Aired July 2019 · The founding trio's last season"
lede: "Season eleven is the founding trio's final season. Three former contestants replace Shannon Bennett as mentors. The grand finale expands to three finalists. Nigella Lawson and Massimo Bottura both appear. Filming spans Brisbane, Noosa, and Rottnest Island."
pull: "The founding trio's last season together — a full-length competition with new mentors, an expanded finale, and three panelists working at the limit of a format they built."
format_summary: "Home cook competition · founding trio final season"
format_caption: "three-finalist grand finale, new mentors"
premiere_caption: "Network Ten · Sunday"
filming_caption: "Melbourne, Brisbane, Noosa, Rottnest Island"
host_caption: "{seasonOrdinalWord} season at the helm"
watch_list:
  - episode_label: "Former contestant mentors"
    body: "Three former high-achieving competitors take the mentor role, replacing Shannon Bennett. Their firsthand knowledge of what this competition asks changes the texture of the guidance they give."
  - episode_label: "Grand finale · three finalists"
    body: "The finale expands from two to three contestants. The structural change adds a pressure layer to the final stretch of competition."
  - episode_label: "All-in Elimination Challenge"
    body: "A new challenge format where the entire active field competes simultaneously. The format makes visible who the field's current ceiling is."
  - episode_label: "Nigella Lawson and Massimo Bottura"
    body: "Both of the founding era's most distinguished recurring guests appear in the same season — an unusual concentration of culinary credibility in a single run."
---

Season eleven closes the founding trio's eleven-year run. Mehigan, Calombaris, and Preston judge their final season with the settled authority of a panel that built the format from scratch. The grand finale expands to three finalists, and three former competitors replace Shannon Bennett as mentors. Nigella Lawson and Massimo Bottura both appear. The season is a complete and capable competition, but it carries the weight of a transition — and the run below is stronger than the run above.

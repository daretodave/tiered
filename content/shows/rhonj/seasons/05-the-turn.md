---
show: rhonj
number: 5
slug: the-turn
title: "The Turn"
canonical_position: 5
premiere_date: "2013-07-07"
location: "Montville, New Jersey"
ep_count: 25
host: "Andy Cohen"
eyebrow: "The reconciliation arc — season five, summer 2013"
lede: "Season five asks whether the family conflict that powered the franchise's peak can move toward resolution. The attempted reconciliation arc gives the season a clear direction and an earnest emotional register — softer than what came before, but honest about what it is."
take_h2: "Catching its breath."
pull: "The franchise catching its breath — and doing it with more grace than you'd expect."
premiere_caption: "Bravo · July 2013"
filming_caption: "Montville, New Jersey and surrounding areas"
format_summary: "Social reality, reconciliation arc"
format_caption: "Five-person cast, reconciliation season"
cast_size: 5
cast_size_caption: "Five returning cast members"
---

The franchise's gentlest early season, and it earns that register. Season five attempts to move the family conflict toward resolution — the cast works in good faith, the arc has a direction, and the production handles the emotional temperature shift without forcing drama the season isn't generating. The intensity of the feud era recedes, but the show stays watchable throughout. Not the franchise at its peak pressure, but an honest account of what follows when that pressure begins to ease.

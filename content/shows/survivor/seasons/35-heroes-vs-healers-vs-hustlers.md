---
show: survivor
number: 35
title: Heroes vs. Healers vs. Hustlers
display_title: "<em>Heroes</em><br/>vs. Healers vs. Hustlers"
premiere_date: 2017-09-27
ep_count: 14
location: Mamanuca Islands, Fiji
host: Jeff Probst
format_changes: [three-tribes, archetypal-casting]
canonical_position: 40
eyebrow: "Aired fall 2017 · Filmed in Fiji"
lede: "Three new tribes split by archetypal frame — heroes, healers, hustlers — the casting team's read on what motivates a modern Survivor applicant. Shot on the Mamanucas, the season runs as a clean three-tribe twist-heavy entry."
pull: "Heroes vs. Healers vs. Hustlers is the casting department asking what archetype means in 2017. The cast plays the brief without being defined by it."
filming_caption: "Mamanuca · third consecutive Fiji shoot"
premiere_caption: "CBS · Wednesday 8/7c"
episodes_caption: "39 days in country"
format_summary: "Three tribes · archetypal split"
format_caption: "heroes, healers, hustlers"
cast_size: 18
cast_size_caption: "6 per archetypal tribe"
host_caption: "thirty-fifth season at the helm"
episode_heat: [med, hot, med, hot, med, hot, hot, med, hot, hot, hot, med, hot, hot]
episode_heat_caption: "even pacing across the run"
watch_list:
  - episode_label: "Ep 1 · archetype brief"
    body: "The casting frame lands cleanly — each tribe arrives carrying a coherent internal voice. The hustler tribe in particular reads as a modernization of the older underdog cast slot."
  - episode_label: "Ep 4 · pre-merge texture"
    body: "Confessional time distributes well across the bench. Watch the show trust unfamiliar casting types to carry full talking-head stretches."
  - episode_label: "Ep 8 · merge cycle"
    body: "The merge runs at the dense pace the twist-heavy era had taught the format to deliver. The strategic vocabulary moves quickly."
  - episode_label: "Ep 13 · final stretch"
    body: "The final-tribal mechanics carry the casting frame all the way through. The archetypal premise still reads in the closing arguments."
---

Three tribes split by archetypal frame — heroes, healers,
hustlers — the casting department's read on what motivates a
modern Survivor applicant. Six players per archetype, shot on
the Mamanucas, with a strategic vocabulary at the pace the
twist-heavy era had taught the format to deliver. The cast
plays the brief without being defined by it, and the season
runs as a clean three-tribe entry in a run of Fiji shoots.
Jeff Probst hosts a season that earns its premise.

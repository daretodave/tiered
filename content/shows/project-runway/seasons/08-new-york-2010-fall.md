---
show: project-runway
number: 8
title: New York
canonical_position: 7
premiere_date: 2010-07-29
ep_count: 14
location: New York City
host: Heidi Klum
eyebrow: "Aired summer 2010 · Filmed in Manhattan"
lede: "The first post-return season with a stable creative team. Seventeen designers in a Manhattan workroom, a judges' table operating from muscle memory, and a cast remembered for confrontational personalities."
pull: "The season the show consolidates its second act. The format has its footing back and the casting team takes the room loud."
shape_h2: "Seventeen designers, a new high."
take_h2: "The second act settles in."
premiere_caption: "Lifetime · Thursday 9/8c"
format_summary: "Standard format · 17 designers"
format_caption: "post-return creative stability"
cast_size: 17
cast_size_caption: "Seventeen designers — the franchise's first count above sixteen."
host_caption: "Heidi Klum's eighth season at the helm"
format_changes: []
---

The second-act consolidation. With the network change a season behind it, Project Runway runs a workroom of seventeen confrontational personalities and lets the editing trust the friction. The judges' table reads from muscle memory now; the critique is quicker, the runway shows are tighter. Manhattan is doing its editorial work again. tiered.tv places the season mid-canon because the casting energy is real but the format choices are familiar — a strong, recognizable run rather than a redefining one.

---
show: project-runway
number: 6
title: Los Angeles
canonical_position: 21
premiere_date: 2009-08-20
ep_count: 14
location: Los Angeles, California
host: Heidi Klum
eyebrow: "Aired summer 2009 · Filmed in Los Angeles"
lede: "The relocation experiment. First season on Lifetime, first off the East Coast, and the only run in the show's history filmed away from the Manhattan workroom the format had been built around."
pull: "The detour season. The critical read still calls it a coastal disconnect — the show feels off its rails because it is on the wrong coast."
shape_h2: "Same headcount, a new coast."
take_h2: "The detour, kept for history."
premiere_caption: "Lifetime · Thursday 10/9c"
format_summary: "Standard format · 16 designers"
format_caption: "first Lifetime season, LA-shot"
cast_size: 16
cast_size_caption: "Sixteen designers — the cast size holds steady through the coast move."
host_caption: "Heidi Klum's sixth season at the helm"
format_changes:
  - "Production moves from Bravo to Lifetime"
  - "Workroom relocates from New York to Los Angeles"
---

The detour. Project Runway's only season filmed off the East Coast, and its first under a new network. The LA workroom and the LA garment district both read different on camera — the light is wrong, the texture is wrong, the cast feels half a step out of frame. The judging is intact, the format is intact, but the city is doing none of the editorial work the show usually borrows from Manhattan. Included for historical importance.

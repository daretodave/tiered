---
show: rhod
number: 2
title: "The Expansion"
canonical_position: 2
location: "Dallas, Texas"
host: Andy Cohen
premiere_date: "2017-08-14"
ep_count: 14
aired_year: 2017
episodes: 14
eyebrow: "Aired August–November 2017 · Bravo"
take_h2: "The cast finds its shape."
shape_h2: "Fourteen episodes, three more than the debut."
lede: "The founding four return, joined by two new Dallas women — D'Andra Simmons and Kameron Westcott — expanding the cast to six. It's the configuration that becomes RHOD's most stable ensemble, replacing the debut year's leaner core with a wider social circle."
pull: "Six women, one settled Dallas circle — the cast configuration RHOD rides through its most consistent stretch."
premiere_caption: "Bravo · August 2017"
filming_caption: "Dallas · founding cast expands to six"
format_summary: "Social reality"
format_caption: "Four returning, two new Housewives"
cast_size: 6
cast_size_caption: "Four returning Housewives plus two new additions"
host_caption: "Andy Cohen hosts the reunion"
---

Season two grows RHOD's founding four to six, adding D'Andra Simmons and Kameron Westcott to a returning Cary Deuber, Stephanie Hollman, LeeAnne Locken, and Brandi Redmond. The wider cast gives Dallas's church-and-charity social scene more room to run, and the six-woman configuration proves durable — it's the lineup the show carries, unchanged, into its next season, the closest thing RHOD has to a settled core.

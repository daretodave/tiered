...
- [ ] [HIGH] sign-out broken — `src/components/chrome/HeaderView.tsx:54` links `/auth/logout?returnTo=/`; the Auth0 SDK forwards `returnTo` verbatim as `post_logout_redirect_uri`, so relative `/` → `%2F`, which Auth0 always rejects (must be an absolute URL matching an Allowed Logout URLs entry). Fix: build the logout href with an absolute canonical URL (`canonicalUrl('/')` from `src/lib/seo.ts` → `https://tiered.tv/`) + a unit test asserting the href is absolute. NOTE: full resolution also needs the user to add the absolute URLs to Auth0 → Allowed Logout URLs (commented on #56); the code fix is correct and shippable on its own. Reference #56 in commit body. (#56) (category: bug, source: triage, score: 5.6) — cloud 2026-05-17

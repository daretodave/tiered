...
- [x] Phase 44 — Brand-spelling discipline (lax→strict `BRAND_SPELLING_STRICT` invariant for the `tiered.tv` wordmark). — b8ebba8 Single-tick shape: `collectBrandSpellingIssues` lands in `scripts/content-check.ts` (two regexes — truncation + `tiered.app` TLD bleed — scanning `src/{app,components,lib}/**/*.{ts,tsx}` excluding `__tests__/` and `content/**/*.md`); four extant truncations drained in the same commit (`/mod` description, `/shows/[show]` JSON-LD `name`, `/themes` JSON-LD `name`, `/not-found` link text); strict flips on day one (surface small enough to clean in one pass). `BRAND_TRUNCATION_ALLOWLIST` carries `src/components/shows/ShowsHero.tsx` for the `<em>Tiered.</em>` editorial-adjective wordplay; `BRAND_DOMAIN_INFRA_ALLOWLIST` carries `src/lib/auth0/permissions.ts` for the Auth0 tenant claim namespace. Pass-24's three named violations (`/sign-in`, mailto, `ListDetailTools`) were already drained by prior `/iterate` ticks; this phase generalized the rule + caught the four uncaught siblings + locked them with the verify gate. Promoted from `PHASE_CANDIDATES.md` #12 (score 6.6).
- [x] Phase 45 — Editorial-cliché repetition guard (`CLICHE_REPETITION_STRICT` cross-corpus phrase-frequency invariant). — e539e7e Core machinery (helper, strict flag, three live pattern classes: `measures/measured against` at threshold 3, `at full volume` at threshold 2, `freighted` at threshold 2) all landed reactively across iterate ticks 92ebd74 (pass-25), 470d37c (pass-29), 0e39b59 (pass-30). This phase formalized the registry's promised `{ phrase, threshold, allowlist }` shape by adding the optional `allowlist?: ReadonlyArray<string>` field on `ClichePattern` (exact `where`-string match excluded before the threshold check, mirroring phase 43's `TENURE_ANCHOR_ALLOWLIST` precedent) plus three colocated tests (allowlist match / miss / absent). Today's three patterns ship with no allowlist entries — current corpus already passes. Decision: deferred adding `sets? the bar` / `the bar every` patterns this tick — pre-flight `rg` returns 6 occurrences for `sets? the bar`, above the proposed threshold; a paired drain tick handles them later. Promoted from `PHASE_CANDIDATES.md` #13 (score 6.0).
- [x] Phase 46 — Colocated-test coverage gate extended to `src/app/**` (finish the phase-42 move). — 9ed40d5 The gate's `ROOTS` (`scripts/check-test-colocation.mjs`) gained `'src/app'` alongside `src/components`/`src/lib`/`src/content`; the loop's ~11-tick reactive drain of `src/app/**` files left zero stragglers, so the flip is clean at pickup. CLI side-effects guarded behind `invokedDirectly` so `ROOTS` + `ALLOWLIST` can be imported by tests; three regression tests pin the configured root set, the `src/app` presence, and the allowlist shape. Allowlist stays empty — the brief's pre-flight expectation of allowlisting `src/app/internal/rank-shift-demo/page.tsx` was revisited at pickup and skipped: the internal demo already ships a comprehensive colocated test (prod-leak gate + render contract) that references `../page`. No `march.yml` wiring change needed — phase-42's oversight push already chained `pnpm check:test-colocation` into the cloud call-1 string. Promoted from `PHASE_CANDIDATES.md` #11 (score 5.5); brief: `phase_46_test_colocation_app.md`.

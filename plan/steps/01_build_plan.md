...
- [ ] Phase 27 — Homepage rework to spec (rebuild `/` against `design/tiered.tv · Home.html`: fluid hero, hero stat strip, 3-up featured shows + sub-row + 6 compact tiles, dual-rank callout, themed lists stacked. Featured show already data-driven via `getFeaturedShow()`. Detailed brief: `phase_27_home_rework.md`. **Ship next.**)

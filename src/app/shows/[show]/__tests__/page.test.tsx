...
describe('generateMetadata — known show', () => {
  it('titles the page with the canon, no-spoilers framing', async () => {
    getShowMock.mockReturnValue(makeShow({ name: 'Survivor', slug: 'survivor' }))
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.title).toBe('Survivor — the canon, no spoilers')
  })

  it('points the canonical OpenGraph URL at /shows/<slug>', async () => {
    getShowMock.mockReturnValue(makeShow({ slug: 'top-chef' }))
    const meta = await generateMetadata({ params: { show: 'top-chef' } })
    expect(meta.alternates?.canonical).toBe('https://tiered.tv/shows/top-chef')
  })

  it('does not mark a real show noIndex', async () => {
    getShowMock.mockReturnValue(makeShow())
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.robots).toBeUndefined()
  })

  it('attaches the global feed AND the per-show RSS feed for discovery', async () => {
  ...
  it('points OpenGraph + Twitter images at the show opengraph-image route, not the site default', async () => {
    getShowMock.mockReturnValue(makeShow({ slug: 'top-chef' }))
    const meta = await generateMetadata({ params: { show: 'top-chef' } })
...
// serves the canon ranking and the community vote table off the same
// route, distinguished only by `?view=community`. Metadata never read
// that param, so every community-view visitor (and every crawler) got
// the canon page's title and a canonical pointing at the plain
// /shows/<slug> URL — confirmed systemic across two shows' community
// pages at critique pass 105.
describe('generateMetadata — community view (?view=community)', () => {
  it('titles the community view distinctly from the canon view', async () => {
    getShowMock.mockReturnValue(makeShow({ name: 'Survivor', slug: 'survivor' }))
    const meta = await generateMetadata({
      params: { show: 'survivor' },
  ...
  it('leaves the canon view untouched when no view param is present', async () => {
    ...
    expect(meta.title).toBe('Survivor — the canon, no spoilers')
    expect(meta.alternates?.canonical).toBe('https://tiered.tv/shows/survivor')
  })

  it('leaves the canon view untouched for any non-community view value', async () => {
    getShowMock.mockReturnValue(makeShow({ name: 'Survivor', slug: 'survivor' }))
    const meta = await generateMetadata({
      params: { show: 'survivor' },
      searchParams: Promise.resolve({ view: 'canon' }),
    })
    expect(meta.title).toBe('Survivor — the canon, no spoilers')
  })
})
...
describe('generateMetadata — unknown show', () => {
  it('returns the "Show not found" title noIndexed so a stale link cannot index', async () => {
    getShowMock.mockReturnValue(undefined)
    const meta = await generateMetadata({ params: { show: 'ghost' } })
    expect(meta.title).toBe('Show not found')
    expect(meta.robots).toEqual({ index: false, follow: false })
  })

  it('still points canonical at the requested path so the 404 is canonicalized cleanly', async () => {
    getShowMock.mockReturnValue(undefined)
    const meta = await generateMetadata({ params: { show: 'ghost' } })
...
describe('generateMetadata — description (SEO budget via descriptionFor)', () => {
  it('prefers the curator card_tagline over the full tagline', async () => {
    ...
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.description).toBe('The format that invented itself in episode one.')
  })

  it('trims surrounding whitespace off the chosen card_tagline', async () => {
    getShowMock.mockReturnValue(
      makeShow({ card_tagline: '  Trimmed lede.  ' }),
    )
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.description).toBe('Trimmed lede.')
  })

  it('falls back to the tagline when card_tagline is whitespace-only', async () => {
    ...
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.description).toBe('The real fallback tagline.')
  })

  it('returns a short tagline verbatim (<=160 chars, no card_tagline)', async () => {
    ...
    const meta = await generateMetadata({ params: { show: 'survivor' } })
    expect(meta.description).toBe(tagline)
  })

  it('clips a long tagline at the last word boundary inside 159 chars + ellipsis', async () => {
  ...
  it('strips a trailing comma before the ellipsis so the clip reads clean', async () => {

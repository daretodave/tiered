...
// Phase 10 ships:
//   - middleware that issues pantheon_anon_id cookie on first request
...
test('anonymous first visit to / sets a pantheon_anon_id cookie', async ({ page, context }) => {
  // Wipe cookies so this is a true first visit.
  await context.clearCookies()
  await page.goto('/', { waitUntil: 'domcontentloaded' })

  const cookies = await context.cookies()
  const anon = cookies.find((c) => c.name === 'pantheon_anon_id')
  expect(anon, 'expected pantheon_anon_id cookie to be issued by middleware').toBeDefined()
  // RFC 4122 v4 shape
...
test('the same anon id persists across subsequent requests', async ({ page, context }) => {
  await context.clearCookies()
  await page.goto('/')
  const after1 = (await context.cookies()).find((c) => c.name === 'pantheon_anon_id')?.value
  await page.goto('/shows')
  const after2 = (await context.cookies()).find((c) => c.name === 'pantheon_anon_id')?.value
